...
function readJSON(){
    ...
    for (let i = 0; i < icons.info.length; i++){
        ...
        for(let y = 0; y < icons.info[i].icon.length; y++){
            ...
            subdivision.innerHTML += '<h2>' + icons.info[i].icon[y].name + '</h2>';
            subdivision.innerHTML += '<p>' + icons.info[i].icon[y].description + '</p>';
            subdivision.innerHTML +=  '<img src="' + icons.info[i].icon[y].url + '" alt="Girl in a jacket" width="100" height="100">'; 
        }
    }
}
